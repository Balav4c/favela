    ...
    public function recordAttendance()
{
    ...
    if (strpos($authHeader, 'Bearer') !== false) {
        $tokenParts = explode('Bearer', $authHeader);
    ...
    $sc_id = $data['sc_id'] ?? $this->request->getPost('sc_id');
    $fv_id = $data['fv_id'] ?? $this->request->getPost('fv_id');
    $type  = $data['type'] ?? $this->request->getPost('type'); // "check_in" or "check_out"

    // Validate required fields
    if (empty($sc_id) || empty($fv_id) || empty($type)) {
        return $this->failValidationErrors('sc_id, fv_id, and type (check_in/check_out) are required.');
    }

    // Verify token validity
    $tokenModel = new TokenVerifyModel();
    $tokencheck = $tokenModel->verifySecurityToken($token, $sc_id);
    ...
    //Continue attendance marking only if token is valid
     $securityModel = new SecurityModel();
    $assigned = $securityModel->checkSecurity($fv_id, $sc_id);
