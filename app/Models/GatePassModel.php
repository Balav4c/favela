...
class GatePassModel extends Model {
	...
	public function getThisGatePass($gp_id, $fv_id) {
		...
		return $this->db->query("SELECT gp.*, sc.security_name	
							FROM gatepass as gp
							left join security as sc on sc.sc_id = gp.security_gaurd_id
							where gp.gp_id = ".$gp_id." and gp.fv_id = ".$fv_id)->getRow();
	}
	...
	public function getTokenGatePass($accesskey) {
		return $this->db->query("SELECT gp.*, sc.security_name	
							FROM gatepass as gp
							left join security as sc on sc.sc_id = gp.security_gaurd_id
							where gp.token = '".$accesskey."'")->getRow();
	}
}
?>